...
namespace ResumableFunctions.Handler.Core
{
    internal class WaitsProcessor : IWaitsProcessor
    {
        ...
        public async Task FindWorkflowMatchedWaits(
            int functionId,
            long pushedCallId,
            int methodGroupId,
            DateTime pushedCallDate)
        {
            await _backgroundJobExecutor.ExecuteWithLock(
                $"ProcessWorkflowExpectedMatchedWaits_{functionId}_{pushedCallId}",
                async () =>
                {
                    ...
                    foreach (var template in waitTemplates)
                    {
                        var waits = await _waitsRepo.GetPendingWaitsForTemplate(
                            template.Id,
                            _pushedCall.GetMandatoryPart(template.CallMandatoryPartPaths),
                            pushedCallDate,
        ...
        private async Task<bool> ExecuteAfterMatchAction()
        {
            ...
            catch (DbUpdateConcurrencyException ex)
            {
                ...
                _backgroundJobClient.Schedule(() =>
                        FindWorkflowMatchedWaits(_methodWait.RequestedByFunctionId, pushedCallId, _methodWait.MethodGroupToWaitId, _pushedCall.Created),
